...
int main(int argc, char** argv) {
    ...
    // ROI ayarla - interaktif mod veya sabit ROI
    if (args.interactiveMode) {
        std::cout << "İnteraktif mod etkin - İlk görüntü ile ROI seçimi yapılacak..." << std::endl;
        
        // İlk lazer görüntüsünü yükle
        cv::Mat firstLaserImage = cv::imread(laserFiles[0]);
        if (firstLaserImage.empty()) {
            std::cerr << "Hata: İlk lazer görüntüsü yüklenemedi: " << laserFiles[0] << std::endl;
            return 1;
        }
        
        // İnteraktif ROI seçimi ve optimizasyon
        bool roiSuccess = laserDetector.optimizeROIAndDetectLaser(firstLaserImage);
        if (!roiSuccess) {
            std::cerr << "ROI seçimi iptal edildi veya başarısız oldu!" << std::endl;
            return 1;
        }
        
        std::cout << "İnteraktif ROI seçimi tamamlandı." << std::endl;
    } else if (args.useROI) {
        // Komut satırından verilen ROI'yi kullan
        laserDetector.setROI(args.roiX, args.roiY, args.roiWidth, args.roiHeight);
        std::cout << "Komut satırı ROI kullanılıyor: x=" << args.roiX << ", y=" << args.roiY 
                 << ", genişlik=" << args.roiWidth << ", yükseklik=" << args.roiHeight << std::endl;
    } else {
        // Sabit ROI kullan (varsayılan)
    ...
    for (int i = 0; i < totalImages; i++) {
        // İlerleme göster
        if (i % 10 == 0) {
            std::cout << "Renk görüntüsü kaydediliyor: " << i << "/" << totalImages << " (" 
                     << (i * 100 / totalImages) << "% )\r" << std::flush;
        }
        
        // Dosya varlığını kontrol et (görüntüyü yüklemeden)
        if (!fs::exists(colorFiles[i])) {
            std::cerr << "\nHata: Renk görüntüsü bulunamadı: " << colorFiles[i] << std::endl;
            continue;
        }
        ...
        // Renk bilgisini ekle (dosya yolu olarak - batch processing için)
        colorMapper.addColorData(colorFiles[i], angle);
    }
